...
func removeShipment(data: [String:Any]) throws -> RequestHandler {
    return {
        request, response in
        ...
        response.setHeader(.contentType, value: "application/json")
        // Tracking a shipment using incoming JSON
        response.appendBody(string: Shipments().deleteShipment(withJSONRequest: request.postBodyString ?? "Empty Body"))
        // Signalling that the request is completed
        response.completed()
    }
}

func updateShipment(data: [String:Any]) throws -> RequestHandler {
    return {
        request, response in
        ...
        response.setHeader(.contentType, value: "application/json")
        // Tracking a shipment using incoming JSON
        response.appendBody(string: Shipments().updateShipment(withJSONRequest: request.postBodyString ?? "Empty Body"))
        // Signalling that the request is completed
        response.completed()
    }
}

func setShipmentDelivered(data: [String:Any]) throws -> RequestHandler {
    return {
        request, response in
        ...
        response.setHeader(.contentType, value: "application/json")
        // Tracking a shipment using incoming JSON
        response.appendBody(string: Shipments().setShipmentDelivered(withJSONRequest: request.postBodyString ?? "Empty Body"))
        // Signalling that the request is completed
        response.completed()
    }
}
